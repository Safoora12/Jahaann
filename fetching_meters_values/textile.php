...
$query = "SELECT DISTINCT id FROM textilemil";
...
if ($result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $query = "SELECT * FROM textilemil WHERE id = $meter_id";
        ...
        if ($data_result && mysqli_num_rows($data_result) > 0) {
            ...
            ${'textile_' . $meter_id . '_name'} = $meter_row['name'];
            ${'textile_' . $meter_id . '_MinVolt'} = $meter_row['minVolt'];
            ${'textile_' . $meter_id . '_MaxVolt'} = $meter_row['maxVolt'];
            ${'textile_' . $meter_id . '_MinCurrnt'} = $meter_row['minCurrnt'];
            ${'textile_' . $meter_id . '_MaxCurrnt'} = $meter_row['maxCurrnt'];
            ${'textile_' . $meter_id . '_MinEnergy'} = $meter_row['minEnergy'];
            ${'textile_' . $meter_id . '_MaxEnergy'} = $meter_row['maxEnergy'];
            ${'textile_' . $meter_id . '_Refresh'} = $meter_row['refresh'];      
