...
$query = "SELECT DISTINCT id FROM papermill";
...
if ($result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $query = "SELECT * FROM papermill WHERE id = $meter_id";
        ...
        if ($data_result && mysqli_num_rows($data_result) > 0) {
            ...
            ${'paper_' . $meter_id . '_name'} = $meter_row['name'];
            ${'paper_' . $meter_id . '_MinVolt'} = $meter_row['minVolt'];
            ${'paper_' . $meter_id . '_MaxVolt'} = $meter_row['maxVolt'];
            ${'paper_' . $meter_id . '_MinCurrnt'} = $meter_row['minCurrnt'];
            ${'paper_' . $meter_id . '_MaxCurrnt'} = $meter_row['maxCurrnt'];
            ${'paper_' . $meter_id . '_MinEnergy'} = $meter_row['minEnergy'];
            ${'paper_' . $meter_id . '_MaxEnergy'} = $meter_row['maxEnergy'];
            ${'paper_' . $meter_id . '_Refresh'} = $meter_row['refresh'];      
